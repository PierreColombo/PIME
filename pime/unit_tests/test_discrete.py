...
from pime.divergence import DISCRETE_ESTIMATORS
...
class TestDiscreteMethods(unittest.TestCase):
    def test_alpha(self):
        for alpha in tqdm([-2, -1.01, -0.99, -0.01, 0.01, 0.99, 1.01, 2], "Alpha Div"):
            ...
            assert torch.sum(
                torch.isclose(
                    alpha_div.predict(random_vector, random_vector),
            ...
            assert torch.sum(
                torch.isclose(
                    alpha_entropy.predict(batched_uniform_tensor),
                    torch.zeros(batch_size),
                    atol=1e-07,
                )
            )
    ...
    def test_beta(self):
        for beta in tqdm([-2, -1.01, -0.99, -0.01, 0.01, 0.99, 1.01, 2], "Beta Div"):
            ...
            assert torch.sum(
                torch.isclose(
                    beta_div.predict(random_vector, random_vector),
                    torch.zeros(batch_size),
                    atol=1e-05,
                )
            )
            ...
            assert torch.sum(
                torch.isclose(
                    beta_entropy.predict(batched_uniform_tensor),
                    torch.zeros(batch_size),
                    atol=1e-05,
                )
            )
    ...
    def test_renyi(self):
        for alpha in tqdm([-2, -1.01, -0.99, -0.01, 0.01, 0.99, 1.01, 2], "Renyi Div"):
            ...
            assert torch.sum(
                torch.isclose(
                    renyi_div.predict(random_vector, random_vector),
                    torch.zeros(batch_size),
                    atol=1e-07,
                )
            )
            ...
            assert torch.sum(
                torch.isclose(
                    renyi_entropy.predict(batched_uniform_tensor),
                    torch.zeros(batch_size),
                    atol=1e-07,
                )
            )
        ...
        for alpha in tqdm([0.2, 0.677, 2], "AB Div"):
            for beta in tqdm([0.5], "AB Div"):
                if alpha + beta != 0:
                    ...
                    ab_entropy = DISCRETE_ESTIMATORS["entropy"](
                        "test_ab_entropy",
                        DISCRETE_ESTIMATORS["ab_div"],
                        alpha=alpha,
                        beta=beta,
                    )
                    ...
                    assert torch.sum(
                        torch.isclose(
                            ab_div.predict(random_vector, random_vector),
                            torch.zeros(batch_size),
                            atol=1e-7,
                        )
                    )
                    ...
                    assert torch.sum(
                        torch.isclose(
                            ab_entropy.predict(batched_uniform_tensor),
                            torch.zeros(batch_size),
                            atol=1e-7,
                        )
                    )
    ...
    def test_lp(self):
        for p in tqdm([-1, 3, 4], "Renyi Div"):
            ...
            assert torch.sum(
                torch.isclose(
                    lp.predict(random_vector, random_vector),
                    torch.zeros(batch_size),
                    atol=1e-07,
                )
            )
            ...
            assert torch.sum(
                torch.isclose(
                    lp_uni.predict(batched_uniform_tensor),
                    torch.zeros(batch_size),
                    atol=1e-07,
                )
            )
    ...
    def test_fisher_rao(self):
        ...
        assert torch.sum(
            torch.isclose(
                fisher_rao.predict(random_vector, random_vector),
                torch.zeros(batch_size),
                atol=1e-07,
            )
        )
        ...
        assert torch.sum(
            torch.isclose(
                fisher_rao_uni.predict(batched_uniform_tensor),
                torch.zeros(batch_size),
                atol=1e-07,
            )
        )
    ...
    def test_kl(self):
        ...
        assert torch.sum(
            torch.isclose(
                kl.predict(random_vector, random_vector),
                torch.zeros(batch_size),
                atol=1e-07,
            )
        )
        ...
        assert torch.sum(
            torch.isclose(
                shanon_entropy.predict(batched_uniform_tensor),
                torch.zeros(batch_size),
                atol=1e-07,
            )
        )
