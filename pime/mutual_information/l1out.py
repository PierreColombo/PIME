...
class L1OutUB(nn.Module):  # naive upper bound
    ...
    def forward(self, x_samples: Tensor, y_samples: Tensor) -> Tensor:
        ...
        all_probs = (-((y_samples_1 - mu_1) ** 2) / 2.0 / logvar_1.exp() - logvar_1 / 2.0).sum(dim=-1)  # [nsample, nsample]

        diag_mask = torch.ones([batch_size]).diag().unsqueeze(-1).to(self.device) * (-20.0)
        negative = utils.log_sum_exp(all_probs + diag_mask, dim=0) - np.log(batch_size - 1.0)  # [nsample]
