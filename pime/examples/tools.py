...
def plot_entropy(filename, show=False, save=True):
    ...
    plt.step(
        list(data_to_plot["x"]),
        data_to_plot["loss_true_entropy"],
        label="True",
        color="black",
        linewidth=6,
    )
    ...
    _, xmax = np.min(data_to_plot["x"]), np.max(data_to_plot["x"])
...
def plot_mi_bias_var_mse(filename, pdf_out=None, save=True, show=False):    # noqa: C
    if pdf_out is None:
    ...
    for mi in MIs:
        mi_est_values = df[df["estimate_true_mi"] == mi]
...
def plot_mi_run(filename, pdf_out=None, save=True, show=False):
    ...
    yaxis_mi = np.array(df["estimate_true_mi"])
    ...
    for i, model_name in enumerate(model_list):
        ...
        if i == 0:
            plt.plot(mis_smooth, c=p1.get_color(), label="$\hat I$")    # noqa: W605
