...
from pime.misc.utils import set_seed
from pime.continuous_gaussian import MI_CONTINUOUS_ESTIMATORS
...
def main(cubic):    # noqa: C
    ...
    for seed in range(args.reps):
        ...
        for model_name in tqdm(model_list, "Models"):
            model = MI_CONTINUOUS_ESTIMATORS[model_name](sample_dim, sample_dim, hidden_size).to(device)  # TODO : check that
...
def get_args():
    ...
    parser.add_argument("--batch_size", type=int, required=False, default=128, help="Size of batches")
    parser.add_argument(
        "--training_steps",
        type=int,
        required=False,
